...
def main():
    ...
    unknown = sorted(set(target_obs.gene.astype(str)) - set(train.obs.gene.astype(str)))
    if unknown and args.unseen_policy == "error":
    ...
    contract.update(
        ...
        architecture="native 2048-wide/3-layer/60-latent; 1000-step linear DDIM eta=0",
        selection="official validation denoising MSE of EMA model; no test outcomes",
    )
    ...
    print(f"Squidiff native training on {train.shape}; unknown test IDs: {unknown}", flush=True)
